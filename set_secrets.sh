...
echo setting crossref password
export STORAGE_ACCOUNT=$(cat ./crossref_password.txt)
sed -i $(printf 's:@crossref-password@:%s:g' "$STORAGE_ACCOUNT") /app/dspace/config/local.cfg
