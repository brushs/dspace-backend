...
import org.hibernate.type.StandardBasicTypes;
     ...
     */
    public class CustomItemDAOImpl extends AbstractHibernateDSODAO<Item> implements ItemDAO {
        private static final Logger log = org.apache.logging.log4j.LogManager.getLogger(org.dspace.content.dao.impl.ItemDAOImpl.class);

        protected CustomItemDAOImpl() {
            super();
        }
        ...
        public Iterator<Item> findMissingThenOldestByField(Context context,
            ...
            // 1) Grab up to `limit` items that DO NOT have any value for the given metadata field
            final String missingHql =
                    "SELECT i " +
                            "FROM Item i " +
                            "WHERE i.inArchive = :inArchive " +
                            "AND NOT EXISTS (" +
                            "  SELECT 1 FROM MetadataValue mv " +
                            "  WHERE mv.dSpaceObject = i AND mv.metadataField = :field" +
                            ") " +
                            "ORDER BY i.id ASC";

            Query missingQ = createQuery(context, missingHql);
            missingQ.setParameter("inArchive", inArchive);
            missingQ.setParameter("field", metadataField);
            missingQ.setMaxResults(limit);

            @SuppressWarnings("unchecked")
            List<Item> results = (List<Item>) missingQ.getResultList();

            // 2) If we still need more, fetch items that DO have the field, ordered by the oldest value
            if (results.size() < limit) {
                int remaining = limit - results.size();

                // We want each Item only once, even if it has multiple values.
                // We GROUP BY the item and ORDER BY MIN(mv.value) (oldest).
                // IMPORTANT: This assumes the date values are stored in a sortable ISO format (e.g., yyyy-MM-dd or ISO-8601).
                // If your dates are not ISO-sortable strings, you may need a DB-specific cast/convert here.
                String withFieldHql =
                        "SELECT i " +
                                "FROM Item i " +
                                "JOIN i.metadata mv WITH mv.metadataField = :field " +
                                "WHERE i.inArchive = :inArchive " +
                                (results.isEmpty() ? "" : "AND i.id NOT IN (:already) ") +
                                "GROUP BY i " +
                                "ORDER BY MIN(mv.value) ASC, i.id ASC";

                Query withFieldQ = createQuery(context, withFieldHql);
                withFieldQ.setParameter("inArchive", inArchive);
                withFieldQ.setParameter("field", metadataField);

                if (!results.isEmpty()) {
                    List<UUID> already = results.stream()
                            .map(Item::getID)   // returns UUID in DSpace 7.x
                            .collect(java.util.stream.Collectors.toList());

                    withFieldQ.setParameter("already", already);
                }

                withFieldQ.setMaxResults(remaining);

                @SuppressWarnings("unchecked")
                List<Item> topUp = (List<Item>) withFieldQ.getResultList();

                results.addAll(topUp);
            }

            // 3) Hand back an Iterator<Item> (no extra DB roundtrips)
        ...
        enum OP {
            ...
            not_equals {
                public Criterion buildPredicate(String val, String regexClause) {
                    return org.dspace.content.dao.impl.ItemDAOImpl.OP.equals.buildPredicate(val, regexClause);
                }
            },
            like {
                public Criterion buildPredicate(String val, String regexClause) {
            ...
            not_like {
                public Criterion buildPredicate(String val, String regexClause) {
                    return org.dspace.content.dao.impl.ItemDAOImpl.OP.like.buildPredicate(val, regexClause);
                }
            },
            contains {
                public Criterion buildPredicate(String val, String regexClause) {
            ...
            doesnt_contain {
                public Criterion buildPredicate(String val, String regexClause) {
                    return org.dspace.content.dao.impl.ItemDAOImpl.OP.contains.buildPredicate(val, regexClause);
                }
            },
            exists {
                public Criterion buildPredicate(String val, String regexClause) {
            ...
            doesnt_exist {
                public Criterion buildPredicate(String val, String regexClause) {
                    return org.dspace.content.dao.impl.ItemDAOImpl.OP.exists.buildPredicate(val, regexClause);
                }
            },
            matches {
                public Criterion buildPredicate(String val, String regexClause) {
            ...
            doesnt_match {
                public Criterion buildPredicate(String val, String regexClause) {
                    return org.dspace.content.dao.impl.ItemDAOImpl.OP.matches.buildPredicate(val, regexClause);
                }
        ...
        public Iterator<Item> findByMetadataQuery(Context context, List<List<MetadataField>> listFieldList,
            ...
            for (int i = 0; i < index; i++) {
                org.dspace.content.dao.impl.ItemDAOImpl.OP op = org.dspace.content.dao.impl.ItemDAOImpl.OP.valueOf(query_op.get(i));
                if (op == null) {
                    log.warn("Skipping Invalid Operator: " + query_op.get(i));
                    continue;
                }

                if (op == org.dspace.content.dao.impl.ItemDAOImpl.OP.matches || op == org.dspace.content.dao.impl.ItemDAOImpl.OP.doesnt_match) {
                ...
                if (op == org.dspace.content.dao.impl.ItemDAOImpl.OP.exists || op == org.dspace.content.dao.impl.ItemDAOImpl.OP.equals || op == org.dspace.content.dao.impl.ItemDAOImpl.OP.like || op == org.dspace.content.dao.impl.ItemDAOImpl.OP.contains || op == org.dspace.content.dao.impl.ItemDAOImpl.OP.matches) {
        ...
        @Override
        public Iterator<Item> findArchivedByCollection(Context context, org.dspace.content.Collection collection, Integer limit,
        ...
        @Override
        public Iterator<Item> findArchivedByCollectionExcludingOwning(Context context, org.dspace.content.Collection collection, Integer limit,
                                                                      Integer offset) throws SQLException {
            CriteriaBuilder criteriaBuilder = getCriteriaBuilder(context);
            javax.persistence.criteria.CriteriaQuery criteriaQuery = getCriteriaQuery(criteriaBuilder, Item.class);
        ...
        @Override
        public int countArchivedByCollectionExcludingOwning(Context context, org.dspace.content.Collection collection) throws SQLException {
            CriteriaBuilder criteriaBuilder = getCriteriaBuilder(context);
            javax.persistence.criteria.CriteriaQuery criteriaQuery = getCriteriaQuery(criteriaBuilder, Item.class);
            Root<Item> itemRoot = criteriaQuery.from(Item.class);
            criteriaQuery.select(itemRoot);
        ...
        @Override
        public Iterator<Item> findAllByCollection(Context context, org.dspace.content.Collection collection) throws SQLException {
            Query query = createQuery(context,
        ...
        @Override
        public Iterator<Item> findAllByCollection(Context context, org.dspace.content.Collection collection, Integer limit, Integer offset)
                throws SQLException {
            Query query = createQuery(context,
                    "select i from Item i join i.collections c WHERE :collection IN c ORDER BY i.id");
            query.setParameter("collection", collection);
        ...
        @Override
        public int countItems(Context context, org.dspace.content.Collection collection, boolean includeArchived, boolean includeWithdrawn)
                throws SQLException {
            Query query = createQuery(context,
