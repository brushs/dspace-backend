 ...
 */
public class CrossRefDOIIdentifierProvider extends FilteredIdentifierProvider implements DOIIdentifierProvider {
    private static final Logger log = LoggerFactory.getLogger(CrossRefDOIIdentifierProvider.class);
     ...
     */
    protected CrossRefDOIIdentifierProvider() {
    }
     ...
     *                   {@link DOIService#formatIdentifier(String)}.
...
//                "AND resource_id = ? AND ((status != ? AND status != ?) OR status IS NULL)";
    ...
    @Override
    public void processCallback(Context context, String batchId, String retrieveUrl) throws SQLException {

        DOI doi = doiService.findByBatchId(context, batchId);
        if (null == doi) {
            throw new IllegalArgumentException("DOI for Batch ID: " + batchId + " not found");
        }
