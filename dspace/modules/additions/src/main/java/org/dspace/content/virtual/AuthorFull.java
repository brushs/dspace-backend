 ...
 */
public class AuthorFull implements VirtualMetadataConfiguration {
    ...
    public List<String> getValues(Context context, Item item) {
        ...
        for (String s : value) {
            ...
            if (s.contentEquals("person.givenName.full") && StringUtils.isNotEmpty(resultString)) {
                fullname = true;
            }
        }
        ...
        String result = "";
        if (fullname) {
            result = resultValues.get(0) + ", " + resultValues.get(1) + " (" + resultValues.get(2) + ")";
        } else {
            result = StringUtils.join(resultValues, this.getSeparator());
        }
